import sax, { SAXParser, Tag } from 'sax'
...
const isEmpty = (thing: {} | null | undefined) => (typeof thing === 'object') && (thing != null) && (Object.keys(thing).length === 0)

const processItem = (processors: any, item: string, key: string | null = null) => {
...
class Parser extends events {
  ...
  assignOrPush(obj: any[] | any, key: string, newValue: any) {
    if (!(key in obj)) {
  ...
  reset() {
    ...
    this.saxParser.onerror = (error: any) => {
    ...
    this.saxParser.onopentag = (node: Tag) => {
      const obj = {};
      obj[charkey] = "";
    ...
    this.saxParser.onclosetag = () => {
      ...
      let node: { [x: string]: any } | undefined;
      let obj = stack.pop();
    ...
    this.saxParser.oncdata = (text: any) => {
      const s = ontext(text);
      if (s) {
        s.cdata = true;
        return s.cdata;
  ...
  resultObject(arg0: string, resultObject: any) {
    throw new Error("Method not implemented.");
  }

  parseString(str: string, cb: ((arg0: null, arg1: undefined) => void) | null = null) {
   ...
   * @param {string} str
   */
  parseStringPromise(str: any) {
    return new Promise((resolve, reject) => {
      return this.parseString(str, (err: any, value: unknown) => {
        if (err) {
          return reject(err);
        } else {
...
const parseString = (str: any, a: any, b: null | Function = null) => {
...
const parseStringPromise = (str: any, a: any) => {
  let options: any
  if (typeof a === 'object') {
    options = a
  }
